...
| `--thin` | `-t` | `2` | 稀疏采样间隔 |
| `--lambda` | - | 空 | 统一的软约束惩罚强度（设置后覆盖分赛制 λ） |
| `--lambda-percent` | - | `50.0` | 百分比制（S3-27）的软约束惩罚强度 |
| `--lambda-rank` | - | `3.0` | 排名制（S1-2, S28+）的软约束惩罚强度 |
| `--no-method-specific-lambda` | - | `False` | 禁用分赛制 λ，改用统一 `--lambda` |
...
#### `--lambda`（惩罚强度）
- 控制软约束的严格程度（值越大越“硬”）
- 由于 percent 与 rank 的违约值量纲不同（percent 约 0.0x，rank 为整数 1,2,...），默认使用分赛制 λ：
  - 百分比制：`--lambda-percent`（默认 50）
  - 排名制：`--lambda-rank`（默认 3）
- 若希望两类赛季完全同一口径，可用统一 `--lambda` 覆盖（或用 `--no-method-specific-lambda`）
...
```json
{
  ...
  "config": {                    // 使用的配置
    ...
    "burn_in": 2000,
    "thin": 2,
    "use_method_specific_lambda": true,
