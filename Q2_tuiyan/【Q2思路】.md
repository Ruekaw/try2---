...
对每个核心统计周、每个宇宙方法，基于 10,000 次模拟淘汰结果，统计：

### 5.1 Reversal Rate（翻转率）
与历史真实淘汰者不一致的概率：
$$P(\text{SimElim} \neq \text{ActualElim})$$

### 5.2 Tech-Vulnerability（技术冤死率）
当周评委总分最高者被淘汰的概率：
$$P(\text{SimElim} = \arg\max(J))$$
该值越小，说明规则越“尊重技术”。

### 5.3 Popularity Dominance（人气被冤死率/人气保护性）
当周粉丝份额第一名被淘汰的概率：
$$P(\text{SimElim} = \arg\max(F_s))$$
该值越小，说明规则越“照顾流量/大众意愿”。

> 说明：Alignment Score 可视为 $1-\text{ReversalRate}$，不额外引入新指标体系。

---

## 6. 个案追踪（题目点名的 Jerry Rice & Bobby Bones）
...
- 跑 tracker：对 Jerry Rice（S2）与 Bobby Bones（S27）输出逐周淘汰概率曲线与关键周解释。
