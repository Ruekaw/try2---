...
### 5.2 最终推荐（公平优先 + 约束 early > late）

为了满足“公平优先”的同时避免参数看起来“越到后期越偏评委”，我们加入一个可解释的约束：

- $w_{early} > w_{late}$：后期不进一步提高粉丝权重（保证“后期更看技术”的叙事一致性）。

在 `daws_save` 且 `w_early > w_late` 的网格子集中，我们以 **最小化 `tech_vulnerability` 为主目标**，并额外要求 `popularity_vulnerability ≤ 0.02` 以避免对“人气强者”的过度牺牲。

**最终推荐参数（公平优先主方案）**：

- 规则：DAWS-Save
- 参数：$w_{early}=0.35,\; w_{late}=0.325,\; N_{cut}=6,\; save=\text{True}$

对应总体指标（实跑 `--eval --save` 输出，见 [../outputs/q4_new_system_fair_pick_early_gt_late/core_metrics_overall.csv](../outputs/q4_new_system_fair_pick_early_gt_late/core_metrics_overall.csv)）：

- `daws_save`：`reversal_rate ≈ 0.3759`，`tech_vulnerability ≈ 0.00383`，`popularity_vulnerability ≈ 0.01709`

写作解读：

- 相对 `percent_save`：技术冤死风险进一步降低（更“公平”），但观众侧冤死风险略上升。
- 相对 `rank_save`：大幅降低对观众侧的极端牺牲（rank_save 的 popularity_vulnerability 极高）。

> 注：同一套参数下 `daws_direct` 的 `reversal_rate` 更低（更贴近历史淘汰），可作为“备选折衷方案”在正文中顺带呈现。
...
下面以 S27 的 Bobby Bones 为例，说明 DAWS 能如何“更平滑地缓解争议”。我们将淘汰概率按周画成折线（见本次公平优先参数输出图：[../outputs/q4_new_system_fair_pick_early_gt_late/trackers/tracker_Bobby_Bones.png](../outputs/q4_new_system_fair_pick_early_gt_late/trackers/tracker_Bobby_Bones.png)）。

**对比分析：DAWS vs. 现实争议**

- **现实争议的制度背景**：Bobby Bones 的争议核心在于评委评价长期偏弱，但粉丝基础极强；在“百分比法（Percent-Direct）”下，他很难被送入危险区，最终夺冠。
- **看绿线（percent_direct）**：整季风险几乎贴近 0，意味着在旧规则下他几乎不会被淘汰，这与“争议发生”的叙事高度一致。
- **看橙线（daws_save）**：
  - 在 S27W1–W3，淘汰概率依然很低：主方案满足 $w_{early}>w_{late}$（早期粉丝权重略高、后期略降低），允许人气选手在前期保留以维持收视与话题。
  - **转折点出现在 W4 与 W8**：随着进入后期阶段（并叠加 Save 的“Bottom-2 再由评委决定”机制），Bobby Bones 的淘汰风险显著抬升；到 W8，`daws_save` 的淘汰概率为 0.5587。
  - 含义：如果采用 DAWS-Save，他在半决赛/决赛前被送入 Bottom-2 的概率会大幅增加，从而显著降低“弱技术但强人气直接夺冠”的争议风险。
- **对比 Rank 方案的“下手力度”**：紫线（rank_direct）在关键周会给出更高的淘汰风险，能更激进地纠偏，但也更可能让“人气型选手”过早离场，影响节目商业价值；棕线（rank_save）在末期对该类选手的惩罚则更为极端。
- **DAWS-Save 更平滑**：橙线在前期保持低风险、后期逐步抬升，体现“公平（保护技术强者）与兴奋（保留观众参与）之间”的连续折衷，而不是在 Rank / Percent 两套制度之间二选一。
